...
PI = M.pi
CONSTANTES = {'CC48':(12952888.0864, 0.0523598775598299, 0.743166306071,
                      11676255.9949, 0.081819191, 6378137, 1700000),
              'CC49':(13754395.7452, 0.0523598775598299, 0.754731385179,
                      11626445.9012, 0.081819191, 6378137, 1700000),
              'L1'  :(5657616.674, 0.0407923443319766, 0.7604059656,
                      11603796.98, 0.08248325676, 6378249.2, 600000),
              'L2'  :(6199695.768, 0.0407923443319766, 0.7289686274,
                      11745793.39, 0.08248325676, 6378249.2, 600000)}
...
#        self.cf = -99
        ...
        if 'CC' in proj1 or 'CC' in proj2:
            for i in open(repertoire+"/"+GRILLE_IGN, 'r'):   # chargement de la grille IGN
                vals = i.split(";")
                self.grilles_ign[(int(float(vals[0])*10+self.oxign+.5),
                                  int(float(vals[1])*10+self.oyign+.5))] =\
        ...
        self.description = proj1+'->'+proj2+' sens grille cus: '+str(sens) + '\n' +\
                           str(self.from_proj) + '\n-> ' + str(self.to_proj) +\
                           '\ngrille ign'+ str(self.grille_ign)
    ...
    def calcule_point_proj(self, x_cour, y_cour, tol=TOLDEF):
        ...
        lam, phi = self.from_proj(x_cour, y_cour, tol) if self.from_proj else\
                    (x_cour*PI/180, y_cour*PI/180)
...
#        print ('apr grille ign ',lam1,phi1)
        (x_proj, y_proj) = self.to_proj(lam1, phi1) if self.to_proj else (lam1*180/PI, phi1*180/PI)
...
#        print ('dans geo_en',X,Y)
    ...
    def xyz_geo(self, x_ca, y_ca, z_ca, v_a, exc, tol):
        ...
        exc2 = exc*exc
        phi1 = M.atan(z_ca/(dist*(1-((v_a*exc2)/M.sqrt(x_ca*x_ca+y_ca*y_ca+z_ca*z_ca)))))
        phi = 999999
#        print ('calcul', phi1,phi,abs(phi1-phi)<tol)
        while abs(phi1-phi) >= tol:
            phi = phi1
            phi1 = M.atan(z_ca/dist*pow(1-(v_a*(exc2)*M.cos(phi))/
                                        (dist*M.sqrt(1-exc2*M.sin(phi)**2)), -1))
    ...
    def optimise_grilleign(self):
        ...
        for noeud in self.grilles_ign:
            ...
            try:
                ...
                continue
            self.grilles_ign_opt[noeud] = (xNO, yNO, zNO, xSE, ySE, zSE,
                                           xSO, ySO, zSO, xNE, yNE, zNE)
    ...
    def calcule_point_grille_ign(self, x_orig, y_orig, z_orig, lam_r, phi_r):
        ...
        try:
            x_no, y_no, z_no, x_se, y_se, z_se, x_so, y_so, z_so, x_ne, y_ne, z_ne =\
            self.grilles_ign_opt[codelam_so, codephi_so]
# position relative dans la maille (on travaille en x 10 comme cela la maille vaut 1)
            ...
            x_grid = (1-xint)*(1-yint)*x_so+(1-xint)*yint*x_no+xint*(1-yint)*x_se+xint*yint*x_ne
            y_grid = (1-xint)*(1-yint)*y_so+(1-xint)*yint*y_no+xint*(1-yint)*y_se+xint*yint*y_ne
            z_grid = (1-xint)*(1-yint)*z_so+(1-xint)*yint*z_no+xint*(1-yint)*z_se+xint*yint*z_ne
...
#        print ('grille_cus', xg, yg)
    ...
    def lamb_to_cc_grille_ign(self, lam_r, phi_r, tol=TOLDEF):
        ...
        x_trans, y_trans, z_trans = self.calcule_point_grille_ign(x_cart, y_cart,
                                                                  z_cart, lamt, phit)
    ...
    def cc_to_lamb_grille_ign(self, lam_r, phi_r, tol=TOLDEF):
        ...
        x_grid, y_grid, z_grid = self.calcule_point_grille_ign(x_cart, y_cart, z_cart,\
                                                               lam_r, phi_r)
...
def param(v_a, ext, _, p_0, p_1, p_2):
    ...
    liso1 = M.log(M.tan(PI/4+p_1/2)*pow((1-ext*M.sin(p_1))/(1+ext*M.sin(p_1)), ext/2))
    liso2 = M.log(M.tan(PI/4+p_2/2)*pow((1-ext*M.sin(p_2))/(1+ext*M.sin(p_2)), ext/2))
    liso0 = M.log(M.tan(PI/4+p_0/2)*pow((1-ext*M.sin(p_0))/(1+ext*M.sin(p_0)), ext/2))
