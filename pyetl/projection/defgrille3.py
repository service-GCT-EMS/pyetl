...
#        self.ymax = -1e99
    ...
    def set_emprise(self, x_ref, y_ref):
        ...
        self.emprise = (min(x_ref, self.emprise[0]), min(y_ref, self.emprise[1]),
                        max(x_ref, self.emprise[2]), max(y_ref, self.emprise[3]))
...
#        print ("dg3:emprise", self.xmin, self.xmax, self.ymin, self.ymax, self.nom)
    ...
    def calcule_coin(self, x_lamb, y_lamb):
        ...
        return (int(x_lamb/self.pas), int(y_lamb/self.pas))# code de la maille contenant un point
...
#            eby = (pos_y*ey4+(1-pos_y)*ey2)
            ecart_x = (pos_y*ex4+(1-pos_y)*ex2)*pos_x+(pos_y*ex3+(1-pos_y)*ex1)*(1-pos_x)
            ecart_y = (pos_y*ey4+(1-pos_y)*ey2)*pos_x+(pos_y*ey3+(1-pos_y)*ey1)*(1-pos_x)
