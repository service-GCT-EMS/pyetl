...
#                   groupe : S simple / M multiple / H hstore / D dynamique / P variable moteur
    relist = {'A':(r'^'+adef+r'$', 'A', 'S'), #variable
              ...
              'LT':(r'^schema:T:([A-Z]+)$', 'L', 'M'), #liste par type
...
#            print("enregistrement",nom,clef_sec,priorite)
    ...
    def __repr__(self):
        return 'fonction_traitement '+self.module+":"+self.nom +\
                ':'+self.definition.__repr__()+self.work.__repr__()
...
def controle_pattern(pattern, noms):
    """validation"""
    definition_champs = {nom:DefinitionAttribut(nom, pat)
                         for nom, pat in zip(noms, pattern.split(";")) if nom != 'commande'}
    if [print(i.ident, 'c: pattern inconnu', i.pattern) for i in definition_champs.values()
            if i.nature == 'erreur']:
        return False
...
#                        print ('enregistrement',nom[2:],desc["#pattern"])
        ...
        #        print ('definitions',definition)
                ...
                stockage[clef].subfonc(nom, fonction, description, definition_champs,
                                       groupe_sortie, clef_sec, style)
...
def regwarnings(module, nom, store, storetype):
    ...
    if nom in store:
        print('!!!!!!!!!!!attention la fonction ', module, nom,
              '\n a deja ete enregistree dans', storetype, store[nom])
...
#        print ('traitement ',module.__name__, nom)
        ...
        if pref in simple_prefix:
            reghelpers(nom_module, nom_fonction, fonction, stockage,
                       simple_prefix[pref])
...
def complete_fonction(sbf, store):
    ...
    if sbf.definition["sortie"].nature == 'G': # c'est une definition de groupe
        ...
        sbf.fonctions_sortie = {k:stockage[k] for k in stockage
...
#            print ('fonction schema ',i.nom, i.description.get('#schema'))
            i.fonction_schema = get_fonction(i.description.get('#schema', [''])[0],
                                             store, 'fschema')
            i.helper = get_fonction(i.description.get('#shelper', [''])[0],
                                    store, "sh")
#        print ("detecte groupe",grouperef,sbf.nom,len(sbf.fonctions_sortie))
    set_helper(sbf, store, "h")

    if sbf.description.get("#shelper"):
        sbf.shelper = get_fonction(sbf.description.get("#shelper", [''])[0],
                                   store, "sh")
...
#    print ('fonction schema ',sbf.nom, sbf.description.get('#schema'))
    sbf.fonction_schema = get_fonction(sbf.description.get('#schema', [""])[0],
                                       store, 'fschema')
...
#    for module in NOMS_MODULES:
    ...
    simple_prefix = {'h':"helper fonction", 'sh':"helper stockage",
                     'selh':"helper selecteur", 'fschema':"schemas"}
