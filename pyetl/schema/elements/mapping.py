...
class MatchClasses(object):
    ...
    def _match_fuzzy(self, qual=0.5):
        ...
        while a_traiter and upperdest:
            ...
            if vmax == 0:
                print('impossible de matcher le reste', a_traiter.values(),
                      upperdest.values())
                break
    ...
    def set_map(self, correspondance, auto=2, qual=0.5):
        ...
        print('generation de correspondance ', directmatch, casematch, fuzzymatch,
              reste)
    ...
    def map_attributs(self, obj):
        ...
        obj.attributs = {self.att_source.get(nom, nom):valeur
...
def match_classe(classe, schema_sortie, qual):
    ...
    if classe_sortie:
        attmap = MatchClasses(classe, classe_sortie,
...
class Mapping(object):
    ...
    def mapping_schema(self, classes, fusion):
        ...
        if fusion:
            ...
            def_mapping.extend([';'.join(i + classes[i].ident_origine+
                                         (str(classes[i].objcnt
                                              if classes[i].objcnt
                                              else classes[i].getinfo('objcnt_init')), ))
                                for i in sorted(classes) if classes[i].a_sortir])
