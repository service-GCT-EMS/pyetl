...
def numconv(iter_fich):
    ''' retourne un entier '''
    return int(next(iter_fich))/1000.
...
def gestion_noeuds(obj, attr, attsup, iter_gy, unit):
    ...
    if tk1 == "0": #arc non orienté
        ...
        obj.geom_v.setpoint([cx1, cy1], calculeangle(cx1, cy1, cx2, cy2),
                            2, longueur=longueur)
        obj.infogeom()
...
#                                    obj.schema.info["type_geom"] = obj.attributs['#type_geom']
                ...
                # decodage des geometries
                    ...
                    obj_ouvert = addpoints25d(obj, iter_gy, valz, 2,
                                              dimension=3, type_geom='2', regle=regle)
                elif val == '3':
                    type_geom = '2'                  # ligne
                    dimension = 2
                    dimcoord = 2
...
#                    print('gy:objet a traiter ', obj)
                    ...
                    if dimcoord == 2:
                        ...
                        obj.geom_v.addpoint([int(val)/unit, numconv(iter_gy),
                                             numconv(iter_gy)], dimension)
                        # lecture de coordonnees
