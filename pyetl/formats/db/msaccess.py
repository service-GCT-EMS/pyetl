...
TYPES_A = {"T":"T", 'VARCHAR':"T", "VARCHAR2":"T", "CLOB":"T", "CHAR":"T",
           ...
           "DATETIME":"D",
           "ROWID":"E",
           "DOUBLE PRECISION":"F", "NUMERIC":"N", 'DOUBLE':'F', "CURRENCY":"N",
           "BOOLEEN":"T", "BOOLEAN":"T",
...
#        fkeys=dict()
        ...
        for tabledef in cur.tables():
            if (tabledef.table_schem if tabledef.table_schem else '',
                    tabledef.table_name) in self.tables:
...
#        print ('msaccess ',[i for i in cur.columns()])
        return [(cd.table_schem if cd.table_schem else '', cd.table_name,
                 cd.column_name,
                 cd.remarks if cd.remarks else '', cd.type_name, 'non', 'non', '',
                 'non' if cd.nullable else 'oui', '', 0, cd.ordinal_position, '', '',
                 pkeys.get((cd.table_name, cd.column_name), ''), '', '',
                 cd.column_size, cd.decimal_digits) for cd in cur.columns()
