...
class DbConnect(object):
    ...
    tabledef = namedtuple("tabledef",
                          ("nom_groupe", "nom_classe", "alias_classe", "type_geometrique",
                           "dimension", "nb_obj", "type_table", "index_geometrique",
                           "clef_primaire", "index", "clef_etrangere"))
    attdef = namedtuple("attdef",
...
#                data = {'geom':geometrie}
            ...
            if self.debug > 2:
                print('debug: database: requete de selection geo',
                      requete, data)
...
#                print ('type inconnu',at,deftype,'par defaut')
            cretable.append('\t'+attname+' '+self.types_db.get(attype, deftype)+defaut+",")
            if sql_conf:
