...
#            print('identifiant clef etrangere :', idfk)
            if idfk not in self.schema.classes:
                if self.connection and self.connection.schemabase and\
                   idfk not in self.connection.schemabase.classes:
    ...
    def cree_comments(self, classe, groupe, nom):
        ...
        if al2:
            if type_table == "V":
                ...
                comments.append("COMMENT ON MATERIALIZED VIEW "+table+
                                " IS '"+al2+"';")
            else:
...
#            print('creation commentaire',atname, alias )
            if alias and alias != al2:
                comments.append("COMMENT ON COLUMN "+table+'.'+atname+" IS '"+alias+"';")
        return comments
...
#                    and self.schema.classes[i].type_table in 'mv'}
            print('get_vues_base: ', len(vues_utilisees), ' definies dans le schema',
                  self.schema.nom, len(vues_schema))
        ...
        if self.connection and self.connection.schemabase and not self.basic:
            return {i:vues_base[i] for i in liste if i in vues_base
                    and self.schema.classes[i].type_table in 'mv'}
        return set()
...
#        codecinfo = "--########### encodage fichier "+cod+'->'+dbcod+' ###(controle éèàç)####\n'
        idschema = "-- ############ nom:"+self.schema.nom+' ## type_base:'+self.dialecte+\
                    ' ## mode:'+('basique' if self.basic else 'std') +' ###############\n'
        self.role = role
        if liste is None:
        ...
        liste_ftables = [i for i in liste if self.schema.classes[i].type_table.upper() == "F"
                         and not self.basic]
...
#  ===============traitement deselements specifiques================
    ...
    #                        continue
                    if niveau[j] >= niv_ref and j != ident and j in vues_base\
    ...
    def vue_to_sql(self, ident, definition, is_mat):
        '''creation des vues '''
        if is_mat:
            return 'CREATE MATERIALIZED VIEW '+'.'.join(ident) +' AS\n'+\
                    definition[:-1] + '\n WITH NO DATA;\n'
