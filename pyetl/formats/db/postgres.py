...
TYPES_PG = {"T":"text", "texte":"text", '':"text", 't':"text", 'A':'text',
            "E":"integer", "entier":"integer",
            "EL":"bigint", "el":'bigint', 'entier_long':'bigint',
            "D":"timestamp", "d":"timestamp",
            "H":"hstore", "h":"hstore", "hstore":"hstore",
            "F":"float", "reel":"float", "float":"float", "flottant":"float", "f":"float",
            "date":"timestamp", "booleen":"boolean", "B":"boolean", "b":"boolean",
            "N":"numeric", 'n':'numeric',
            "S":"serial NOT NULL",
...
#        print (list(i for i in self._def_fonction_triggers() if "admin_sigli" in i))
    ...
    def _def_vues(self):
        return {(i[0], i[1]):(i[2], i[3])
                for i in self.request(REQS["info_vues"])}

    def _def_fonctions_trigger(self):
        return {(i[0], i[1]):i[2]
                for i in self.request(REQS["def_fonctions_trigger"])}

    def _def_ftables(self):
        return {i[0]:i[1:]
                for i in self.request(REQS["info_tables_distantes"])}

    def _def_triggers(self):
        def_trigg = dict()
#        print ('triggers')
#        print (self.request(REQS["info_triggers"]))
        def_trigg['_header'] = ['table', 'nom', 'condition', 'action',
                                'declencheur', 'timing', 'event']
...
#        print('fonctions a garder', fonctions_a_garder)
        els["def_fonctions_trigger"] = {i:j for i, j in els["def_fonctions_trigger"].items()
