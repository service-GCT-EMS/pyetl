WITH t AS
				( SELECT c.oid AS identifiant,
						...
						i.indexrelid,
						(c.oid::text || ':'::text) || COALESCE(array_to_string(i.indkey, ':'::text), ':'::text) AS clef,
						row_number() OVER (PARTITION BY c.oid,
																										i.indrelid) AS num_index,
						i.indkey,
						champ.champ,
					...
					WHERE n.nspname <> 'information_schema'::name --AND n.nspname <> 'public'::name
	...
	t3 AS
				(SELECT t2.identifiant,
					...
					FROM pg_attribute a,
						t2
					WHERE a.attrelid = t2.identifiant
									AND a.attnum > 0
	...
	t4 AS
				(SELECT t3.identifiant,
						...
						string_agg(t3.def_index, ' '::text
																	ORDER BY t3.def_index) AS def_index,
...
SELECT t4.nomschema,
	...
	CASE
					WHEN (
															(SELECT pg_type.typtype
																FROM pg_type
																WHERE t4.atttypid = pg_type.oid)) = 'e'::"char" THEN 'text'::text
					WHEN (format_type(t4.atttypid, t4.atttypmod) = 'integer'
											AND
															(select pg_get_serial_sequence(quote_ident(t4.nomschema)||'.'||quote_ident(t4.nomtable),quote_ident(t4.attname)) IS NOT Null)) THEN 'serial'
					WHEN (format_type(t4.atttypid, t4.atttypmod) = 'bigint'
											AND
															(select pg_get_serial_sequence(quote_ident(t4.nomschema)||'.'||quote_ident(t4.nomtable),quote_ident(t4.attname)) IS NOT Null)) THEN 'bigserial'
	...
	'non'::text AS multiple,
				...
				(SELECT "substring"(pg_get_expr(d.adbin, d.adrelid),
																									...
																									128) AS "substring"
					FROM pg_attrdef d
					WHERE d.adrelid = t4.identifiant
									AND d.adnum = t4.attnum
									AND t4.atthasdef) AS defaut,
	CASE
	...
	CASE
					WHEN (
															(SELECT pg_type.typtype
																FROM pg_type
																WHERE t4.atttypid = pg_type.oid)) = 'e'::"char" THEN
	...
	COALESCE(
														(SELECT CASE
															...
															WHERE a.attrelid = t4.identifiant
																			AND a.attname = 'geometrie'::name), 0) AS dimension, --'fin'::text AS fin,
t4.attnum AS num_attribut,
	...
	t4.pk AS clef_primaire,
				...
				(SELECT (n.nspname::text || '.'::text) || t.relname::text
					...
					WHERE c.conrelid = t4.identifiant
				...
				(SELECT a1.attname
					...
					WHERE c.conrelid = t4.identifiant
									AND c.contype = 'f'::"char"
									AND (t4.attnum = ANY (c.conkey))
									AND a1.attrelid = c.confrelid
									AND a1.attnum = c.confkey[1]) AS cible_clef,
	0 as taille,
	0 as decimales
FROM t4;
