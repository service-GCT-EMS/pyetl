...
#            print ('calcul emprise', xmin, ymin, xmax, ymax, geom_v)
            self.attributs.update([("#longueur", str(geom_v.longueur)),
            ...
            self.attributs.update([("#longueur", ''), ("#points", ''),
                                   ("#xmin", ''), ("#xmax", ''),
                                   ("#ymin", ''), ("#ymax", ''),
                                   ("#type_geom", '0'), ('#dimension', '0'),
    ...
    def __repr__(self):
        ...
        return ('objet'+virtuel+str(self.ido)+":clone"+str(self.copie)+ " : "+
    ...
    def __json_if__(self):
        ...
        liste = self.liste_attributs if self.liste_attributs\
...
#        print ('jsonio recupere ',geom)
        atts = ',\n'.join(['"'+i+'": "'+self.attributs.get(i, '').replace('"', '\\"')+'"'
                           for i in liste])
        if geom:
    ...
    def __geo_interface__(self):
        ...
        if use_noms_courts:
            goif = {"id": attributs.get("#gid", str(self.ido)),
                    "properties": {self.casefold(self.schema.attributs[i].nom_court):\
                                   attributs[i] if i in self.attributs else None for i in liste},
                    "geometry": geom}
        else:
            goif = {"id": attributs.get("#gid", str(self.ido)),
                    "properties": {self.casefold(i):attributs[i] if i in attributs
                                                    else None for i in liste},
                    "geometry": geom}
...
#            print ('attmap', self.schema.schema.nom, self.schema.attmap)
    ...
    def initattr(self):
        ...
        for i in self.schema.get_liste_attributs(sys=True):
            self.attributs[i] = self.schema.attributs[i].defaut\
                                if self.schema.attributs[i].defaut else ''
