...
filemanager_blueprint = Blueprint(
    "flaskfilemanager", __name__, static_folder="RichFilemanager", static_url_path=""
)
...
def init(
    ...
    if custom_config_json_path:
        ...
        log.info(
            "File Manager using custom config.json path: {}".format(
    ...
    if custom_init_js_path:
        ...
        log.info(
            "File Manager using custom init.js path: {}".format(custom_init_js_path)
        )
...
def get_file(path=None, content=None):
    ...
    if extension in ["gif", "jpg", "jpeg", "png"]:
        try:
            ...
            log.exception(
                'Error loading image "{}" to get width and height'.format(os_file_path)
            )
    ...
    return {"id": path, "type": file_type, "attributes": attributes}
...
def edit_file():
    ...
    content = "".join(open(os_file_path).readlines()).decode()
...
def download_file():
    ...
    return send_from_directory(
        _FILE_PATH, web_path_to_local(web_path), as_attachment=True
    )
...
def get_image():
    ...
    if thumbnail:
        ...
        thumbnail_image = image.resize(64, 64)
