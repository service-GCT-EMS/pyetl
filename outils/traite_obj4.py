...
#                    print ("bord",bord)
    ...
    def follow_edge(self, pt, verbose=False):
        ...
        prec=debut
#        print ('============================controle=====',15834,self.liens.get(15834),15829,self.liens.get(15829),15830,self.liens.get(15830))
        while suite!=debut:
            bord.append(suite)
...
#        self.cycle2(trou)
            ...
            for i in range(0, taille-1, 2):
                ...
                if connect != 2:
                    if connect == 3 and taille == 3:
                        ...
                        print ('connectivite trouvee ',connect, triangle, len(trou))
                        tr2.append(trou[i])
                        tr2.append(trou[i+1])
                        cycle = True
                else:
        ...
        if tr2:
            # nettoyage
            for i in range(start_len,start_num+start_len):
        ...
        while suite:
            ...
            for i in petits_trous:
                if len(i)>4:
                    suite = True
                    i[:] = self.trytoclose(i)
            taille = sum(len(i) for i in petits_trous)
            if taille == 0:
                suite = False
            if taille == taillep:
                suite = False
                print ('sortie en etteurs ')
            taillep = taille
#                     self.verifie_sens()
        self.bords = bords2
